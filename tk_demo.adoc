...
I want to demonstrate the value of insider trading data in https://www.sec.gov/edgar/searchedgar/companysearch.html[EDGAR], the SEC's corporate filings database. To do so, you will interact with a database that has been pre-populated with 1.1 million filings taken from EDGAR. In the next few slides you will interact with the database  
